...
Functions:
    - get_fixtures(): Reads the fixtures data from a CSV file, drops rows with NaN values, and returns the DataFrame.
    - get_weeks_fixtures(fixtures_df, matchweek_no): Filters the fixtures DataFrame to return only the fixtures for the specified match week.
    - get_this_week(df): Finds and returns the fixtures for the current week based on today's date.
...
def get_weeks_fixtures(fixtures_df: pd.DataFrame, matchweek_no: int) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: DataFrame containing fixtures for the specified match week.
    """
