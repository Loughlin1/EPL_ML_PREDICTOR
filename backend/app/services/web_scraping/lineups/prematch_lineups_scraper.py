...
def print_element(element):
    print(
        f"Tag: {element.tag_name}, Class: {element.get_attribute('class')}, Text: {element.get_attribute('textContent')}"
    )
...
def find_match_link(driver: webdriver.Chrome, home_team: str, away_team: str):
    try:
        element = driver.find_element(
            By.XPATH,
            f"//*[contains(text(),'{home_team}') and contains(text(), '{away_team}')]",
        )
...
def find_formations(driver: webdriver.Chrome) -> tuple[str, str]:
    try:
        ...
        home_formation_value_element = home_formation_element.find_element(
            By.XPATH, "following-sibling::span[1]"
        )
        home_formation_value = home_formation_value_element.text
        home_formation = (
            home_formation_value.replace(" ", "") if home_formation_value else None
        )
        # print(f"Home Formation found: {home_formation}")

        away_formation_element = elements[1]
        away_formation_value_element = away_formation_element.find_element(
            By.XPATH, "following-sibling::span[1]"
        )
        away_formation_value = away_formation_value_element.text
        away_formation = (
            away_formation_value.replace(" ", "") if away_formation_value else None
        )
...
def find_lineups(driver: webdriver.Chrome, team: str):
    def get_player_names_from_list(player_items: list):
        ...
        for player in player_items:
            player_wrapper = player.find_element(
                By.XPATH, ".//div[contains(@class, 'PlayerWrapper')]"
            )
            player_name_wrapper = player_wrapper.find_element(
                By.XPATH, ".//span[contains(@class, 'PlayerNameWrapper')]"
            )
            name = player_name_wrapper.find_element(
                By.XPATH, ".//span[contains(@class, 'PlayerName')]"
            )
    ...
    try:
        ...
        starting_div = first_child_div.find_element(
            By.XPATH, ".//div[contains(@class, 'TeamPlayers')]"
        )
        subs_section = first_child_div.find_element(
            By.XPATH, ".//section[contains(@class, 'Substitutes')]"
        )

        # Team Lineup
        player_list = starting_div.find_element(
            By.XPATH, ".//ul[contains(@class, 'PlayerList')]"
        )
        player_items = player_list.find_elements(By.TAG_NAME, "li")
        team_lineup = get_player_names_from_list(player_items)

        # Substitutes
        subs_list_div = subs_section.find_element(
            By.XPATH, ".//div[contains(@class, 'TeamPlayers')]"
        )
        subs_list = subs_list_div.find_element(
            By.XPATH, ".//ul[contains(@class, 'PlayerList')]"
        )
...
def scrape_lineups_for_match(home_team: str, away_team: str, month: str) -> dict:
    ...
    Args:
        ...
        month (str): The month of the match e.g. "2025-08"
    Returns:
...
if __name__ == "__main__":
    print(scrape_lineups_for_match("Ipswich Town", "West Ham United", "2025-05"))
